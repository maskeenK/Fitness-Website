...
app.put("/home/foods", isLoggedIn, isVerified, function(req, res){
	...
	User.findById(userId).populate({path: "diet.food"}).exec(function(err, user){
		for(var i=0; i<user.diet.length; i++){
			...
			// Time: n^2 (n is number of foods in checkout list)
			if(!flag){
...
app.get("/profile", isLoggedIn, isVerified, function(req, res){
	...
	Profile.findById(profileId, function(err, foundProfile){
		// time: O(1) ,Constant time due to hash map
		if(err){
			console.log(err);
		}
		else{
...
app.get("/diet", isLoggedIn, isVerified, function(req, res){
	...
	Food.distinct('categoryTag', function(err, categories){
		...
		//category query
		if(req.query.tags){
			// Time O(n*l) {number of categories - n, length of category name - l}
		...
		Food.find({name: nameQuery, categoryTag: {$in: categoryQuery}, proteinTag: proteinQuery, fatTag: fatQuery, carbohydratesTag: carbohydratesQuery}).skip(perPage*pageNumber - perPage).limit(perPage).exec(function(err, allFood){
			Time: O(n) n is number of query types
			...
			User.find({}, function(err, users){
				users.forEach(function(user){
					User.findById(user._id, function(err, user){
						...
						user.diet.find(function(d, index){
							{
							Time: O(n*m) {n: no of users, m: no of items in checkout List}
								if(d.food == foodId){
									idx = index;
									return true;
								}
